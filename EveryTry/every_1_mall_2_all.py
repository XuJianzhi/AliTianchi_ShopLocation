...
#提取
#data['mall_id'].groupby(data['mall_id']).count()==data['mall_id'].groupby(data['mall_id']).count().max()
data_use=data[data['mall_id']=='m_1175']
...
mall.to_csv(way_write+'mall.csv')

joblib.dump(scale, way_write+'scale_1.model')

#保存mall列表
mall_list=pd.Series(data['mall_id'].unique())
mall_list.to_csv(way_write+'mall_list.csv')
