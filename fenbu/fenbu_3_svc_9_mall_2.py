...
parameters=pd.DataFrame(columns=['loop_number','C','gamma','correct_rate'])
#parameters=pd.DataFrame(columns=['loop_number','C','gamma','correct_rate','member'])
for i in range(11,len(mall_list)):
	...
	y4=y3
	way_tempt_scale='C:/Users/Administrator/Desktop/ali/data/3_tempt/svc/scale_'+mall_list[i]+'.model'
	joblib.dump(scale,way_tempt_scale)

	#list1=pd.Series([0.001,0.01,0.1,1,10,100,1000])
	#list1=pd.Series([0.1,1,10,100,1000])
	#list1=pd.Series([0.001,0.1,10,1000])
	list1=pd.Series([10,100,1000,10000])
	#list1=pd.Series([100,1000,10000,100000,1000000])
	...
	for ii in list1:
		for jj in list2:
			...
			print('------------------------------------------------------------------------------')
			time_now=time.strftime('%H:%M:%S',time.localtime(time.time()))
			print('number_loop='+str(i)+'....mall_id='+mall_list[i]+'....'+str(len(x5))+'....'+time_now)
			#记录最大值
			print(score)
	...
	svc.fit(x5,y5)				
	print('mall_id='+mall_list[i]+',,,,'+time_now+',,,,'+str(len(x5)))
	way_tempt_svc='C:/Users/Administrator/Desktop/ali/data/3_tempt/svc/svc_'+mall_list[i]+'.model'
	joblib.dump(svc,way_tempt_svc)
	
	#保存每个商场的C和gamma参数
	parameters_append=pd.DataFrame([[i,ii_max,jj_max,max]],columns=['loop_number','C','gamma','correct_rate'])
	#parameters_append=pd.DataFrame([[i,ii_max,jj_max,max,len(y5)]],columns=['loop_number','C','gamma','correct_rate','member'])
