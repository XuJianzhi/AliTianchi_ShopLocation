...
#恢复文件三的训练
way_tempt_2='C:/Users/Administrator/Desktop/ali/data/3_tempt/svc_old/same_label.scv'	#########
...
for i in range(25):
	...
	for j in range(40):
		...
		#恢复训练
		way_tempt='C:/Users/Administrator/Desktop/ali/data/3_tempt/svc_old/params2_i'+str(i)+'_j'+str(j)+'.model'		#############
		svc=joblib.load(way_tempt)
		#开始预测
		y3=pd.Series(svc.predict(x3),index=x3.index)
		#储存结果
		result=pd.concat([result,y3])
